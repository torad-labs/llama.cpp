...
static __device__ __forceinline__ void flash_attn_ext_f16_load_mask(
    ...
    if (mask_packed) {
        ...
        const uint16_t * const maskw = (const uint16_t *) mask_base + k_VKQ_0/16;
#pragma unroll
...
#pragma unroll
            for (int w0 = 0; w0 < nwords; w0 += warp_size) {
                ...
                if (w0 + warp_size > nwords && w >= nwords) {
                    break;
                }

                // past i_sup the tile is outside the row: no read, the cells are unmasked as in the f16 path
                const uint16_t bits = (!oob_check || 16*w < i_sup) ? maskw[int64_t(j_vram)*stride_mask + w] : 0;
                half2 * const dst = (half2 *) (tile_mask + j_sram*(nbatch_fa + 8) + 16*w);
#pragma unroll
                for (int b = 0; b < 8; ++b) {
                    const int i = 16*w + 2*b;
                    const float m0 = (!oob_check || i     < i_sup) && !((bits >> (2*b))     & 1) ? -INFINITY : 0.0f;
                    const float m1 = (!oob_check || i + 1 < i_sup) && !((bits >> (2*b + 1)) & 1) ? -INFINITY : 0.0f;
                    dst[b] = make_half2(m0, m1);
                }
            }
        }
        return;
    }
    const half * const mask_h = mask_base + k_VKQ_0;
    if constexpr (use_cp_async) {
...
#pragma unroll
    for (int c0 = 0; c0 < nchunks; c0 += nwarps*warp_size) {
        ...
        cp_async_cg_16<64>(tile_raw_32 + c*16, KV + int64_t(i)*stride_KV + k*16);
    }
}
...
#pragma unroll
    for (int p0 = 0; p0 < npairs; p0 += nwarps*warp_size) {
        const int p = p0 + threadIdx.y*warp_size + threadIdx.x;

        if (p0 + nwarps*warp_size > npairs && p >= npairs) {
            break;
        }
...
#pragma unroll
            for (int h = 0; h < 2; ++h) {
                const half2 * src = tile_Q + (g + 8*r)*stride_tile_Q + b*(QK4_0/2) + h*(QK4_0/4) + 2*t;
                const float2 v0 = __half22float2(src[0]);
                const float2 v1 = __half22float2(src[1]);
                amax[r] = fmaxf(amax[r], fmaxf(fmaxf(fabsf(v0.x), fabsf(v0.y)), fmaxf(fabsf(v1.x), fabsf(v1.y))));
            }
        }
#pragma unroll
        for (int offset = 1; offset < 4; offset <<= 1) {
            amax[r] = fmaxf(amax[r], __shfl_xor_sync(0xFFFFFFFF, amax[r], offset, warp_size));
        }
        Q8.d[r] = amax[r] / 127.0f;
    }
    const float id[2] = {amax[0] > 0.0f ? 127.0f/amax[0] : 0.0f, amax[1] > 0.0f ? 127.0f/amax[1] : 0.0f};
...
#pragma unroll
            for (int r = 0; r < 2; ++r) {
                ...
                const float2 v0 = __half22float2(src[0]);
                const float2 v1 = __half22float2(src[1]);
                const int q0 = __float2int_rn(v0.x*id[r]);
                const int q1 = __float2int_rn(v0.y*id[r]);
                const int q2 = __float2int_rn(v1.x*id[r]);
                const int q3 = __float2int_rn(v1.y*id[r]);
                sum[r] += q0 + q1 + q2 + q3;
...
#pragma unroll
                for (int l = 0; l < 2; ++l) {
                    ...
                    acc[3] = fmaf(__int_as_float(dot[3]) - bias, dk.y, acc[3]);
                }
            }
            // Element 4h + l of the tile<16, 16, float> is Q column g + 8*(l/2) and K row 8h + 2t + l%2.
            KQ_C[i00/(np*T_C_KQ::J)].x[4*h + 0] = acc[0]*Q8.d[0];
            KQ_C[i00/(np*T_C_KQ::J)].x[4*h + 1] = acc[1]*Q8.d[0];
            KQ_C[i00/(np*T_C_KQ::J)].x[4*h + 2] = acc[2]*Q8.d[1];
            KQ_C[i00/(np*T_C_KQ::J)].x[4*h + 3] = acc[3]*Q8.d[1];
        }
    }
#else
...
static __device__ __forceinline__ void flash_attn_ext_f16_iter(
        ...
        const int jt,
        const int kb0,
        const int kb0_next, // the step the multi-stage pipeline preloads
        const int k_VKQ_sup) {
...
#pragma unroll
    ...
    GGML_UNUSED_VARS(Q_f2, K_h2, V_h2, mask_h, dstk, dstk_fixup,
        ...
        Q_B, Q8, VKQ_C, KQ_max, KQ_rowsum, kb0, kb0_next);
    NO_DEVICE_CODE;
...
#pragma unroll
    ...
    int kb0 = live_bits ? fattn_kv_live_select<warp_size>(live_bits, nwords, kb0_start) : kb0_start;
...
static __global__ void flash_attn_ext_f16(
    ...
    // kbc == k block continuous, current index in continuous ijk space. With KV_live it counts live KV steps: an output tile
    // is its Q tile's live steps, so the blocks split only the KV some row of a tile sees.
    const fattn_kv_live live = fattn_kv_live_view(KV_live, iter_j, ne03, iter_k);
    const int total_work = KV_live ? live.seq_start[ne03] : iter_k*iter_j*iter_z_gqa*ne12*ne03;
    int       kbc      = int64_t(blockIdx.x + 0)*total_work / gridDim.x;
    const int kbc_stop = int64_t(blockIdx.x + 1)*total_work / gridDim.x;
    ...
    while (kbc < kbc_stop) {
        ...
        int tile_start;
        int tile_steps;
        const uint32_t * live_bits = nullptr;
        if (KV_live) {
            const fattn_kv_live_tile tile = fattn_kv_live_find(live, kbc);
            sequence   = tile.sequence;
            z_KV       = tile.zt / iter_z_gqa;
            zt_gqa     = tile.zt - z_KV*iter_z_gqa;
            jt         = tile.jt;
            tile_start = tile.start;
            tile_steps = tile.n;
            live_bits  = live.bits + (sequence*iter_j + jt)*live.nwords;
        } else {
            sequence   =  kbc /(iter_k*iter_j*iter_z_gqa*ne12);
            z_KV       = (kbc - iter_k*iter_j*iter_z_gqa*ne12 * sequence)/(iter_k*iter_j*iter_z_gqa);
            zt_gqa     = (kbc - iter_k*iter_j*iter_z_gqa*ne12 * sequence - iter_k*iter_j*iter_z_gqa * z_KV)/(iter_k*iter_j);
            jt         = (kbc - iter_k*iter_j*iter_z_gqa*ne12 * sequence - iter_k*iter_j*iter_z_gqa * z_KV - iter_k*iter_j * zt_gqa) / iter_k;
            tile_start = kbc - kbc % iter_k;
            tile_steps = iter_k;
        }

        // kb0 == k start index when in the output tile.
        const int  kb0_start = kbc - tile_start;
        ...
        // KV_min: the KV below it is masked for this Q tile, so skipping it leaves the softmax as it is. process_tile
        // always runs its last iteration: a block whose whole range is masked runs one masked iteration of its own.
        int kb0_live = kb0_start;
        if (KV_max) {
