...
| `72a3533` | a backend sampler attaching to a context reserves the scheduler only when the graph with it exceeds the node budget of the last reserve (`graph_max_nodes`); a compute buffer the graph outgrows is still reallocated when the scheduler allocates it. The server attaches a slot's chain at every request launch, so a same-shaped chain re-entering a graph already reserved for it paid ~18 ms, a stall of every slot. Same 48 requests with `-bs`: 51 → 3 reserves (the count without `-bs`), 18 releases, 48 of 48 texts identical to `1576c6a`'s; `test-backend-sampler --test reattach_no_reserve` counts 0 reserves on a reattach, 1 with the switch set | `LLAMA_SAMPLER_ATTACH_RESERVE_LEGACY=1` |
