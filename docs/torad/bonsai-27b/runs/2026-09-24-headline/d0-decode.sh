#!/usr/bin/env bash
# Decode at depth 0 (tg128), where the matrix puts the fork and Prism b10709 within a few percent: A-B-A-B with five
       ...
       tail -1 | awk -F, -v l="$2" '{gsub(/"/,""); print l "\t" $(NF-1) "\t±" $NF}'; }
{
  ...
  GGML_CUDA_CONCAT_TRANSPOSE_SM120_LEGACY=1 GGML_CUDA_SCALE_VEC4_SM120_LEGACY=1 GGML_CUDA_GDN_Q8_CACHE_LEGACY=1 \
  GGML_CUDA_MMVQ_FUSION_SRC1_LEGACY=1 GGML_CUDA_GRAPH_KEY_LEGACY=1 \
