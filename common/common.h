...
struct common_params {
    ...
    bool attn_mask_bits    = false; // bit-packed attention mask (one bit per KV cell) instead of f16; needs flash attention
...
// runs every thread on the NUMA node of the GPUs the model uses, if the host has more nodes and that node a CPU for each CPU backend thread (Linux); common_init_from_params undoes it when layers stay on the CPU
